...
public:
	...
	*/
	...
	void move(Player & player, GameMap & gameMap, bool gotAttacked) override {
	...
	void resolveDices(Player & player, GameMap& gameMap, vector<Player> & players) override
	{
	...
	void diceRoll(Player & player, bool extraDices) override
	{
		bool canRoll = true;
		char rollAgain;
		vector<int> toReroll;
		...
		while (canRoll)
		{
			cout << "Will reroll everything that is not Attack or Destruction." << endl;
